...
router.get("/postslug/:id", async function(req, res, next) {
  ...
  await Posts.findSlug({ slug: req.params.id })
    .then(function(post) {
      res.send(post);
    })
    .catch(next);
...
router.get("/comments/:id", function(req, res, next) {
  Comments.find({ id: req.params.id })
    .then(function(post) {
      res.send(post);
    })
    .catch(next);
...
router.get("/checkthepostid/:id", (req, res, next) => {
  Posts.findSlug({ comments: req.params.id })
    .then(postdata => {
      // POST DATA ICINDE POSTA AIT SEYLER VAR
...
router.put("/votes/:id", function(req, res, next) {
  Comments.findByIdAndUpdate(
    req.params.id,
    { $set: { meta: { upvotes: req.body.meta.upvotes } } },
    { new: true }
  )
    .then(function(post) {
